...
# -*- coding: utf-8 -*-
"""
check_buenos_aires.py — Buenos Aires (SAEZ, Ezeiza): GFS + ICON Global + ECMWF (separados, sin promediar)
...
LAT, LON = -34.8222, -58.5358
STATION = "SAEZ"
WU_COUNTRY = "AR"
# Clave publica embebida en el propio HTML de wunderground.com - la recibe cualquier
# visitante de la pagina, no es privada ni protegida. Buenos Aires resuelve via
# Wunderground (confirmado en el texto de resolucion del mercado de Polymarket).
WU_API_KEY = "e1f10a1e78da46f5b10a1e78da96f525"
TZ = "America/Argentina/Buenos_Aires"
READINGS_FILE = Path("data/buenos_aires_readings.json")
TRACKING_FILE = Path("data/buenos_aires_tracking.json")
STATS_FILE = Path("data/buenos_aires_stats.json")
DASH_SNAPSHOT_FILE = Path("data/dashboard_snapshot_buenos_aires.json")
...
BRAND = "SIGNAL // BUE"
# Tema único de Buenos Aires — acento amarillo. Colores de modelo (ECMWF/ICON) fijos
# en todos los dashboards de todas las ciudades, para reconocer el modelo por color
# sin importar la ciudad. Sudamérica no tiene modelo regional propio en la selección
# del proyecto, así que se usan 3 modelos globales, siguiendo la regla de fallback
# ("sin alternativa regional fuerte -> usar las opciones globales").
C_BG, C_PANEL, C_ACCENT = "#242a34", "#373d45", "#c98500"
C_GFS = "#3987e5"
C_ICON = "#9085e9"
C_ECMWF = "#199e70"
C_REAL, C_TEXT, C_TEXT_DIM, C_GRID = "#ffffff", "#ffffff", "#c3c2b7", "#454b52"

MODELS = {"gfs": "gfs_seamless", "icon_global": "icon_global", "ecmwf": "ecmwf_ifs025"}
MODEL_COLORS = {"gfs": C_GFS, "icon_global": C_ICON, "ecmwf": C_ECMWF}

# Nota: no hay BIAS_TABLE para Buenos Aires todavía — a diferencia de Helsinki/Chengdu,
# no se cuenta con semanas de tracking histórico viento/cielo-vs-error para esta ciudad.
# No inventar una tabla; usar el valor suavizado directo sin corrección de sesgo.
...
def fetch_wu_max_hoy():
    ...
    realmente liquida el mercado (no un proxy como METAR o PWS corregido por sesgo)."""
...
def save_dashboard_snapshot(date, now_str, real_t, models, wind_dir, sky):
    ...
    snap = {
        "city": "Buenos Aires", "brand": BRAND, "date": date, "updated_at": now_str,
...
def fetch_buenos_aires_market_meta(target_date):
    try:
        ...
        for m in r.json():
            ...
            if "buenos aires" not in q: continue
...
def maybe_record_daily_picks(date, probs_by_model, market_meta):
    ...
    msg = [f"📌 BUENOS AIRES — PICKS DEL DÍA — {date}"]
    for p in picks:
...
def render_dashboard(date, now_str, real_t, finals, peak_hours, wind_dir, sky,
    ...
    axh.text(0, 0.2, f"Buenos Aires ({STATION}) · {date} · {now_str}", fontsize=13, color=C_TEXT_DIM, transform=axh.transAxes)
    ...
    path = IMG_DIR / "buenos_aires_dashboard.png"
...
def run_check():
    ...
    print("=" * 60)
    print(f"  BUENOS AIRES — GFS / ICON GLOBAL / ECMWF — {date} ({now_str})")
    print("=" * 60)
    ...
    # for Buenos Aires — there is no hand-tuned BIAS_TABLE for this city yet), so
    # analyze_buenos_aires_accuracy.py can score against the real signal.
    ...
    market_meta = fetch_buenos_aires_market_meta(date)
    chart_rows = []
    if market_meta:
        print("\n  MERCADO EN VIVO:")
