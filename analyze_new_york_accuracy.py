...
# -*- coding: utf-8 -*-
"""
analyze_new_york_accuracy.py — Desde el inicio del tracking en adelante: pronóstico
ECMWF vs. resultado real en Polymarket para Nueva York (NYC). Fuente de pronóstico:
new_york_readings.json (mantenido por check_new_york.py).
"""
...
SEARCH_QUERIES = ["nyc temperature", "nyc", "highest temperature nyc", "NYC°F"]
SMOOTH_N = 3  # must match check_new_york.py's smoothing window

BRAND = "PRECISION REPORT — NYC"
# Mismo tema de Nueva York (acento verde) que check_new_york.py.
C_BG, C_PANEL, C_ACCENT = "#102a34", "#173c45", "#008300"
C_WIN, C_LOSS = "#0ca30c", "#d03b3b"
C_TEXT, C_TEXT_DIM, C_GRID = "#ffffff", "#c3c2b7", "#1f4c56"
...
def parse_date_bucket(question):
    ...
    bucket_match = re.search(r"be (-?\d+)°?f", q)
...
def fetch_all_nyc_markets():
    ...
    print(f"  [DEBUG] {total_events} eventos revisados en {len(SEARCH_QUERIES)} búsquedas | "
          f"{len(seen_market_ids)} mercados únicos de NYC | fechas encontradas: {sorted(found.keys())}")
...
def get_ecmwf_forecast_for_date(date_str):
    ...
    check_new_york.py) 2) fallback: promedio suavizado del crudo."""
    if READINGS_FILE.exists():
        try:
            ...
            if final_key in readings and readings[final_key]:
                return readings[final_key][-1], "check_new_york(final)"
            key = f"{date_str}_ecmwf"
            if key in readings and readings[key]:
                recent = readings[key][-SMOOTH_N:]
                return sum(recent) / len(recent), "check_new_york(smoothed)"
        except Exception:
            pass
...
def collect_rows():
    all_markets = fetch_all_nyc_markets()
    rows = []
...
def render_accuracy_dashboard(rows, now_str):
    ...
    axh.text(0, 0.15, f"NYC · ECMWF vs resultado real · muestra: {len(decided)} · {now_str}",
    ...
    y -= 0.09
    for r in rows[-12:]:
        ecmwf_str = f"{r['last_ecmwf']:.0f}°F" if r["last_ecmwf"] is not None else "s/d"
        pred_str = f"{r['predicted_bucket']}°F" if r["predicted_bucket"] is not None else "—"
        win_str = f"{r['winning_bucket']}°F" if r["winning_bucket"] is not None else ("pendiente" if not r["resolved"] else "?")
    ...
    path = IMG_DIR / "new_york_accuracy_dashboard.png"
...
def main():
    now_str = datetime.now(ZoneInfo("America/New_York")).strftime("%Y-%m-%d %H:%M")
    print("=" * 80)
    print(f"  ANÁLISIS — ECMWF vs RESULTADO REAL (NYC), desde {START_DATE}")
    print("=" * 80)

    rows = collect_rows()
    if not rows:
        print("\n  ⚠️ No se encontró NINGÚN mercado de NYC.")
        return
    ...
    for r in rows:
        ecmwf_str = f"{r['last_ecmwf']:.0f}°F" if r["last_ecmwf"] is not None else "s/d"
        pred_str = f"{r['predicted_bucket']}°F" if r["predicted_bucket"] is not None else "—"
        win_str = f"{r['winning_bucket']}°F" if r["winning_bucket"] is not None else "-"
        result_str = "ACIERTO" if r["hit"] is True else "FALLO" if r["hit"] is False else "-"
    ...
    send_telegram_photo(dash_path, caption=f"📈 Reporte de precisión ECMWF NYC (desde {START_DATE}) — {n_hits} aciertos / {n_misses} fallos ({wr:.0f}%)")
