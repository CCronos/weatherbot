...
# -*- coding: utf-8 -*-
"""
analyze_milan_accuracy.py — Desde el inicio del tracking en adelante: pronóstico
ECMWF vs. resultado real en Polymarket para Milan. Fuente de pronóstico:
milan_readings.json (mantenido por check_milan.py).
"""
...
SEARCH_QUERIES = ["milan temperature", "milan", "highest temperature milan", "Milan°C"]
SMOOTH_N = 3  # must match check_milan.py's smoothing window

BRAND = "PRECISION REPORT — MIL"
# Mismo tema de Milan (acento rojo) que check_milan.py.
C_BG, C_PANEL, C_ACCENT = "#27273e", "#3b3855", "#e66767"
C_WIN, C_LOSS = "#0ca30c", "#d03b3b"
C_TEXT, C_TEXT_DIM, C_GRID = "#ffffff", "#c3c2b7", "#4a4560"
...
def fetch_all_milan_markets():
    ...
    print(f"  [DEBUG] {total_events} eventos revisados en {len(SEARCH_QUERIES)} búsquedas | "
          f"{len(seen_market_ids)} mercados únicos de Milan | fechas encontradas: {sorted(found.keys())}")
...
def get_ecmwf_forecast_for_date(date_str):
    ...
    check_milan.py — Milan no tiene bias-correction todavía) 2) fallback: promedio
    ...
    if READINGS_FILE.exists():
        try:
            ...
            if final_key in readings and readings[final_key]:
                return readings[final_key][-1], "check_milan(final)"
            key = f"{date_str}_ecmwf"
            if key in readings and readings[key]:
                recent = readings[key][-SMOOTH_N:]
                return sum(recent) / len(recent), "check_milan(smoothed)"
        except Exception:
            pass
...
def collect_rows():
    all_markets = fetch_all_milan_markets()
    rows = []
...
def render_accuracy_dashboard(rows, now_str):
    ...
    axh.text(0, 0.15, f"Milan · ECMWF vs resultado real · muestra: {len(decided)} · {now_str}",
    ...
    path = IMG_DIR / "milan_accuracy_dashboard.png"
...
def main():
    now_str = datetime.now(ZoneInfo("Europe/Rome")).strftime("%Y-%m-%d %H:%M")
    print("=" * 80)
    print(f"  ANÁLISIS — ECMWF vs RESULTADO REAL (Milan), desde {START_DATE}")
    print("=" * 80)

    rows = collect_rows()
    if not rows:
        print("\n  ⚠️ No se encontró NINGÚN mercado de Milan.")
        return
    ...
    send_telegram_photo(dash_path, caption=f"📈 Reporte de precisión ECMWF Milan (desde {START_DATE}) — {n_hits} aciertos / {n_misses} fallos ({wr:.0f}%)")
