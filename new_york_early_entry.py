...
# -*- coding: utf-8 -*-
"""
new_york_early_entry.py — Detecta mercados NUEVOS de Nueva York (NYC) apenas se
crean y simula una entrada temprana usando la proyección HRRR+GEM+ECMWF ya
calculada. NO ejecuta compras reales — solo registra y avisa.
"""
...
SEEN_FILE = Path("data/new_york_seen_markets.json")
EARLY_TRACKING_FILE = Path("data/new_york_early_tracking.json")
...
def fetch_new_nyc_markets():
    ...
    return [m for m in markets if "nyc" in (m.get("question") or "").lower()]
...
def process_new_market(m, seen):
    ...
    # Extraer el bucket numérico de la pregunta (ej. "be 82°F" -> 82)
    bucket_match = re.search(r"be (-?\d+)°?F", question, re.IGNORECASE)
    ...
    lines = [
        f"🆕 NUEVO MERCADO NYC — {target_date}",
        question,
    ...
    for name, v in forecasts.items():
        if v is not None:
            p = probs.get(name)
            lines.append(f"{name.upper()} proyecta: {v:.0f}°F" + (f" → prob. bucket {bucket}°F: {p*100:.0f}%" if p is not None else ""))
...
def main():
    print("NYC EARLY ENTRY WATCHER — iniciado")
    ...
    seen = load_seen()
    print(f"  Mercados de NYC ya conocidos: {len(seen)}")

    while True:
        try:
            markets = fetch_new_nyc_markets()
            new_count = 0
            for m in markets:
                mid = str(m.get("id", ""))
                if mid not in seen:
