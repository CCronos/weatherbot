...
# -*- coding: utf-8 -*-
"""
chengdu_early_entry.py — Detecta mercados NUEVOS de Chengdu apenas se crean
y simula una entrada temprana usando la proyección ECMWF+ICON ya calculada.
...
SEEN_FILE = Path("data/chengdu_seen_markets.json")
EARLY_TRACKING_FILE = Path("data/chengdu_early_tracking.json")
...
def bucket_probs(center, sigma, pad=12):
    ...
    para no perder silenciosamente pronósticos fuera de temporada (esto usaba un rango
    fijo 25-45°C hasta ahora, distinto a los otros 5 scripts de entrada temprana, que
    ya usaban este enfoque relativo — corregido para que Chengdu quede consistente)."""
...
def fetch_new_chengdu_markets():
    ...
    chengdu = [m for m in markets if "chengdu" in (m.get("question") or "").lower()]
    return chengdu
...
def process_new_market(m, seen):
    ...
    # Extraer el bucket numérico de la pregunta (ej. "be 38°C" -> 38)
    ...
    ecmwf = fetch_forecast_for_date("ecmwf_ifs025", target_date)
    icon = fetch_forecast_for_date("icon_seamless", target_date)

    ecmwf_p = bucket_probs(ecmwf, SIGMA_DEFAULT).get(bucket, 0.0) if (ecmwf is not None and bucket is not None) else None
    icon_p = bucket_probs(icon, SIGMA_DEFAULT).get(bucket, 0.0) if (icon is not None and bucket is not None) else None

    lines = [
        f"🆕 NUEVO MERCADO CHENGDU — {target_date}",
        question,
        f"Precio apertura: ${price:.3f}" if price is not None else "Precio: s/d",
    ]
    if ecmwf is not None:
        lines.append(f"ECMWF proyecta: {ecmwf:.1f}°C" + (f" → prob. bucket {bucket}°C: {ecmwf_p*100:.0f}%" if ecmwf_p is not None else ""))
    if icon is not None:
        lines.append(f"ICON proyecta: {icon:.1f}°C" + (f" → prob. bucket {bucket}°C: {icon_p*100:.0f}%" if icon_p is not None else ""))

    entered = False
    if price is not None and price <= ENTRY_MAX_PRICE and bucket is not None:
        best_p = max(p for p in [ecmwf_p, icon_p] if p is not None) if (ecmwf_p is not None or icon_p is not None) else None
            ...
            if ev is not None and ev >= MIN_EV_HIGHLIGHT:
                ...
                if real_ask is not None and real_ask <= ENTRY_MAX_PRICE and real_ev is not None and real_ev >= MIN_EV_HIGHLIGHT:
                    ...
                    early.append({
                        ...
                        "ecmwf": ecmwf, "icon": icon,
                        "ecmwf_prob": ecmwf_p, "icon_prob": icon_p,
                        "ev": real_ev,
                        "cost": POSITION_SIZE_USD,
                        "shares": shares,
                        "status": "pending",
                        "entered_at": datetime.now(ZoneInfo("Asia/Shanghai")).strftime("%Y-%m-%d %H:%M:%S"),
                    })
...
def main():
    print("CHENGDU EARLY ENTRY WATCHER — iniciado")
    ...
    seen = load_seen()
    print(f"  Mercados de Chengdu ya conocidos: {len(seen)}")

    while True:
        try:
            markets = fetch_new_chengdu_markets()
            new_count = 0
            for m in markets:
                mid = str(m.get("id", ""))
                if mid not in seen:
