...
# -*- coding: utf-8 -*-
"""
hong_kong_early_entry.py — Detecta mercados NUEVOS de Hong Kong apenas se crean
y simula una entrada temprana usando la proyección JMA-GSM+ICON-Global+ECMWF ya
...
LAT, LON = 22.3020, 114.1742  # Hong Kong Observatory HQ — la fuente de resolución real
TZ = "Asia/Hong_Kong"
...
SEEN_FILE = Path("data/hong_kong_seen_markets.json")
EARLY_TRACKING_FILE = Path("data/hong_kong_early_tracking.json")
...
def fetch_new_hong_kong_markets():
    try:
        r = requests.get(
    ...
    return [m for m in markets if "hong kong" in (m.get("question") or "").lower()]
...
def process_new_market(m, seen):
    ...
    # Extraer el bucket numérico de la pregunta (ej. "be 30°C" -> 30)
    ...
    lines = [
        f"🆕 NUEVO MERCADO HONG KONG — {target_date}",
        question,
...
def main():
    print("HONG KONG EARLY ENTRY WATCHER — iniciado")
    ...
    seen = load_seen()
    print(f"  Mercados de Hong Kong ya conocidos: {len(seen)}")

    while True:
        try:
            markets = fetch_new_hong_kong_markets()
            new_count = 0
            for m in markets:
                mid = str(m.get("id", ""))
                if mid not in seen:
