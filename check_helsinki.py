...
# -*- coding: utf-8 -*-
"""
check_helsinki.py — Helsinki (EFHK): ICON-EU + UKMO + ECMWF (separados, sin promediar)
...
def fetch_wu_max_hoy():
    ...
    realmente liquida el mercado (Helsinki resuelve via Wunderground, confirmado)."""
...
TRACKING_FILE = Path("data/helsinki_tracking.json")
STATS_FILE = Path("data/helsinki_stats.json")
DASH_SNAPSHOT_FILE = Path("data/dashboard_snapshot_helsinki.json")
...
BRAND = "SIGNAL // HEL"
# Tema único de Helsinki — acento azul. Colores de modelo (ECMWF/ICON/UKMO) fijos
...
C_ICON, C_UKMO, C_ECMWF = "#9085e9", "#e66767", "#199e70"
C_REAL, C_TEXT, C_TEXT_DIM, C_GRID = "#ffffff", "#ffffff", "#c3c2b7", "#2c4066"

MODELS = {"icon_eu": "icon_eu", "ukmo": "ukmo_seamless", "ecmwf": "ecmwf_ifs025"}
MODEL_COLORS = {"icon_eu": C_ICON, "ukmo": C_UKMO, "ecmwf": C_ECMWF}

BIAS_TABLE = {
    ("SO", "despejado"): (+1.36, 100, 16),
    ("S",  "parcial"):   (+1.28, 100, 18),
    ("S",  "seco"):      (+1.25, 100, 58),
    ("O",  "parcial"):   (+0.70, 100, 8),
    ("S",  "despejado"): (+1.26, 98, 42),
    ("SO", "seco"):      (+1.23, 97, 33),
    ("NE", None):        (+0.35, 67, 15),
    ("N",  "seco"):      (+0.45, 67, 15),
    ("NE", "seco"):      (+0.45, 67, 9),
    ("E",  "lluvia"):    (+0.58, 67, 12),
    (None, "nublado"):   (+0.48, 71, 52),
    ("NO", "nublado"):   (+0.40, 81, 21),
}
...
def save_dashboard_snapshot(date, now_str, real_t, models, wind_dir, sky):
    ...
    snap = {
        "city": "Helsinki", "brand": BRAND, "date": date, "updated_at": now_str,
...
def fetch_helsinki_market_meta(target_date):
    try:
...
def maybe_record_daily_picks(date, probs_by_model, market_meta):
    ...
    msg = [f"📌 HELSINKI — PICKS DEL DÍA — {date}"]
    for p in picks:
...
def render_dashboard(date, now_str, real_t, finals, peak_hours, wind_dir, sky,
    ...
    axh.text(0, 0.2, f"Helsinki ({STATION}) · {date} · {now_str}", fontsize=13, color=C_TEXT_DIM, transform=axh.transAxes)
    ...
    path = IMG_DIR / "helsinki_dashboard.png"
...
def run_check():
    ...
    print("=" * 60)
    print(f"  HELSINKI — ICON-EU / UKMO / ECMWF — {date} ({now_str})")
    print("=" * 60)
    ...
    for name, peak_t in peaks.items():
        ...
        if bias_info: smooth += bias_info[0]
        finals[name] = smooth
    if bias_info:
        print(f"  [SESGO] {wind_dir or '-'} + {sky or '-'} -> {bias_info[0]:+.2f}°C")

    # Persist the smoothed + bias-corrected ECMWF value actually used for trading, so
    # analyze_helsinki_accuracy.py can score against the real signal, not a raw reading.
    ...
    market_meta = fetch_helsinki_market_meta(date)
    chart_rows = []
    if market_meta:
        print("\n  MERCADO EN VIVO:")
...
def main():
    print(f"{BRAND} — iniciado (paridad completa con Chengdu)")
