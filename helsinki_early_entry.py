...
# -*- coding: utf-8 -*-
"""
helsinki_early_entry.py — Detecta mercados NUEVOS de Helsinki apenas se crean
y simula una entrada temprana usando la proyección ICON-EU+UKMO+ECMWF ya calculada.
...
SEEN_FILE = Path("data/helsinki_seen_markets.json")
EARLY_TRACKING_FILE = Path("data/helsinki_early_tracking.json")
...
def fetch_new_helsinki_markets():
    try:
        r = requests.get(
    ...
    return [m for m in markets if "helsinki" in (m.get("question") or "").lower()]
...
def process_new_market(m, seen):
    ...
    lines = [
        f"🆕 NUEVO MERCADO HELSINKI — {target_date}",
        question,
...
def main():
    print("HELSINKI EARLY ENTRY WATCHER — iniciado")
    ...
    seen = load_seen()
    print(f"  Mercados de Helsinki ya conocidos: {len(seen)}")

    while True:
        try:
            markets = fetch_new_helsinki_markets()
            new_count = 0
            for m in markets:
                mid = str(m.get("id", ""))
                if mid not in seen:
