...
# -*- coding: utf-8 -*-
"""
analyze_hong_kong_accuracy.py — Desde el inicio del tracking en adelante: pronóstico
ECMWF vs. resultado real en Polymarket para Hong Kong. Fuente de pronóstico:
hong_kong_readings.json (mantenido por check_hong_kong.py).
"""
...
SEARCH_QUERIES = ["hong kong temperature", "hong kong", "highest temperature hong kong", "Hong Kong°C"]
SMOOTH_N = 3  # must match check_hong_kong.py's smoothing window

BRAND = "PRECISION REPORT — HKG"
# Mismo tema de Hong Kong (acento magenta) que check_hong_kong.py.
C_BG, C_PANEL, C_ACCENT = "#262541", "#39345a", "#d55181"
C_WIN, C_LOSS = "#0ca30c", "#d03b3b"
C_TEXT, C_TEXT_DIM, C_GRID = "#ffffff", "#c3c2b7", "#473f66"
...
def parse_date_bucket(question):
    ...
    if "highest temperature" not in q:
        # Polymarket tambien publica un mercado de temperatura MINIMA para Hong Kong,
        # con la misma forma de pregunta ("be 27C on July 22") y numeros de bucket que
        # se solapan con los del maximo - sin este filtro, fetch_all_hong_kong_markets
        # los mezclaba bajo la misma fecha+bucket y el minimo terminaba pisando al
        # maximo real en el diccionario (encontrado 2026-07-24 comparando contra el
        # evento oficial: reportaba "ganador 27C" cuando el maximo real fue 33C).
        return None, None
...
def fetch_all_hong_kong_markets():
    found = {}
    seen_market_ids = set()
    total_events = 0
    ...
    print(f"  [DEBUG] {total_events} eventos revisados en {len(SEARCH_QUERIES)} búsquedas | "
          f"{len(seen_market_ids)} mercados únicos de Hong Kong | fechas encontradas: {sorted(found.keys())}")
...
def get_ecmwf_forecast_for_date(date_str):
    """Prioridad: 1) valor final (suavizado, lo que realmente se usa para operar en
    check_hong_kong.py) 2) fallback: promedio suavizado del crudo."""
    if READINGS_FILE.exists():
        try:
            ...
            if final_key in readings and readings[final_key]:
                return readings[final_key][-1], "check_hong_kong(final)"
            key = f"{date_str}_ecmwf"
            if key in readings and readings[key]:
                recent = readings[key][-SMOOTH_N:]
                return sum(recent) / len(recent), "check_hong_kong(smoothed)"
        except Exception:
            pass
...
def collect_rows():
    all_markets = fetch_all_hong_kong_markets()
    rows = []
...
def render_accuracy_dashboard(rows, now_str):
    ...
    axh.text(0, 0.15, f"Hong Kong · ECMWF vs resultado real · muestra: {len(decided)} · {now_str}",
    ...
    path = IMG_DIR / "hong_kong_accuracy_dashboard.png"
...
def main():
    now_str = datetime.now(ZoneInfo("Asia/Hong_Kong")).strftime("%Y-%m-%d %H:%M")
    print("=" * 80)
    print(f"  ANÁLISIS — ECMWF vs RESULTADO REAL (Hong Kong), desde {START_DATE}")
    print("=" * 80)

    rows = collect_rows()
    if not rows:
        print("\n  ⚠️ No se encontró NINGÚN mercado de Hong Kong.")
        return

    print(f"\n  {'Fecha':<12}{'ECMWF':<9}{'Fuente':<24}{'Predicho':<11}{'Ganador':<11}{'Resultado':<11}{'Modo'}")
    print("  " + "-" * 96)
    for r in rows:
        ecmwf_str = f"{r['last_ecmwf']:.1f}°C" if r["last_ecmwf"] is not None else "s/d"
        pred_str = f"{r['predicted_bucket']}°C" if r["predicted_bucket"] is not None else "—"
        ...
        result_str = "ACIERTO" if r["hit"] is True else "FALLO" if r["hit"] is False else "-"
        print(f"  {r['date']:<12}{ecmwf_str:<9}{r['source']:<24}{pred_str:<11}{win_str:<11}{result_str:<11}{r['mode']}")

    dash_path, wr, n_hits, n_misses = render_accuracy_dashboard(rows, now_str)
    send_telegram_photo(dash_path, caption=f"📈 Reporte de precisión ECMWF Hong Kong (desde {START_DATE}) — {n_hits} aciertos / {n_misses} fallos ({wr:.0f}%)")
