...
# -*- coding: utf-8 -*-
"""
buenos_aires_early_entry.py — Detecta mercados NUEVOS de Buenos Aires apenas se crean
y simula una entrada temprana usando la proyección GFS+ICON Global+ECMWF ya calculada.
...
SEEN_FILE = Path("data/buenos_aires_seen_markets.json")
EARLY_TRACKING_FILE = Path("data/buenos_aires_early_tracking.json")
...
def fetch_new_buenos_aires_markets():
    try:
        r = requests.get(
    ...
    return [m for m in markets if "buenos aires" in (m.get("question") or "").lower()]
...
def process_new_market(m, seen):
    ...
    # Extraer el bucket numérico de la pregunta (ej. "be 22°C" -> 22)
    ...
    lines = [
        f"🆕 NUEVO MERCADO BUENOS AIRES — {target_date}",
        question,
    ...
    if price is not None and price <= ENTRY_MAX_PRICE and bucket is not None and valid_probs:
        ...
        if ev is not None and ev >= MIN_EV_HIGHLIGHT:
            # Re-validar contra el precio REAL de compra (bestAsk) antes de registrar la
            # entrada simulada - "price" viene de outcomePrices, que es un precio de
            # referencia/mid, no necesariamente lo que de verdad se pagaria al comprar.
            # Agregado 2026-07-24 tras confirmar en vivo que bestAsk puede ser el DOBLE
            # de outcomePrices en buckets baratos/finos (visto en Milan).
            real_ask = None
            try:
            ...
            if real_ask is None or real_ask > ENTRY_MAX_PRICE:
                lines.append(f"\n— Sin entrada (bestAsk real ${real_ask} fuera de rango) —" if real_ask is not None
                              else "\n— Sin entrada (no se pudo confirmar bestAsk real) —")
                send_telegram("\n".join(lines))
                print("  [TELEGRAM] Alerta enviada.")
                return
            real_ev = calc_ev(best_p, real_ask)
            if real_ev is None or real_ev < MIN_EV_HIGHLIGHT:
                lines.append(f"\n— Sin entrada (EV real {real_ev} con ask ${real_ask:.3f} insuficiente) —")
                send_telegram("\n".join(lines))
                print("  [TELEGRAM] Alerta enviada.")
                return
...
def main():
    print("BUENOS AIRES EARLY ENTRY WATCHER — iniciado")
    ...
    seen = load_seen()
    print(f"  Mercados de Buenos Aires ya conocidos: {len(seen)}")

    while True:
        try:
            markets = fetch_new_buenos_aires_markets()
            new_count = 0
            for m in markets:
                mid = str(m.get("id", ""))
                if mid not in seen:
