(function () {
  ...
  function renderLive() {
    ...
    updEl.textContent = "última lectura: " + edad + " (" + gen.toISOString().slice(0, 16).replace("T", " ") + " UTC)";
    ...
    body.innerHTML = LIVE.cities.map(function (c) {
      ...
      return "<tr>" +
        ...
        "<td>" + (c.model_peak != null ? c.model_peak.toFixed(1) + "°" : "s/d") + "</td>" +
  ...
  input.addEventListener("focus", function () { renderStationList(input.value); });
  ...
  function render() {
    ...
    document.getElementById("ro-sub").innerHTML =
      ...
      " Temporada de datos: <b>" + SEASON_ES[season] + "</b>.";
