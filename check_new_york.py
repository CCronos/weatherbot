...
# -*- coding: utf-8 -*-
"""
check_new_york.py — Nueva York (KLGA): HRRR + GEM + ECMWF (separados, sin promediar)
TAF | Lluvia/tormenta | Ciclo del modelo | Tracking automático
Mercado en grados Fahrenheit — METAR/TAF llegan en Celsius y se convierten.
...
LAT, LON = 40.7772, -73.8726  # LaGuardia — el mercado resuelve en esta estación, no en la ciudad
STATION = "KLGA"
WU_COUNTRY = "US"
...
def fetch_wu_max_hoy():
    ...
    realmente liquida el mercado (NYC resuelve via Wunderground, confirmado)."""
    ...
    url = (
        ...
        f"?apiKey={WU_API_KEY}&units=e&startDate={fecha}"
    )
    try:
        ...
        print(f"  [WARN] WU: {e}")
        return None
READINGS_FILE = Path("data/new_york_readings.json")
TRACKING_FILE = Path("data/new_york_tracking.json")
STATS_FILE = Path("data/new_york_stats.json")
DASH_SNAPSHOT_FILE = Path("data/dashboard_snapshot_new_york.json")
...
SMOOTH_N = 3
MIN_MODEL_PROB = 0.01
SIGMA_DEFAULT = 2.0  # bucket width is 1°F here vs 1°C elsewhere — wider sigma matches bot_v2.py's SIGMA_F
...
BRAND = "SIGNAL // NYC"
# Tema único de Nueva York — acento verde. Colores de modelo (ECMWF) fijos en todos
# los dashboards de todas las ciudades; HRRR/GEM son exclusivos de EE.UU./Canadá.
C_BG, C_PANEL, C_ACCENT = "#102a34", "#173c45", "#008300"
C_HRRR, C_GEM, C_ECMWF = "#d95926", "#c98500", "#199e70"
C_REAL, C_TEXT, C_TEXT_DIM, C_GRID = "#ffffff", "#ffffff", "#c3c2b7", "#1f4c56"

# "hrrr" se pide como gfs_seamless en Open-Meteo — mismo criterio ya usado en bot_v2.py
# ("HRRR+GFS seamless — best option for US"); no hay un id de modelo HRRR puro separado.
MODELS = {"hrrr": "gfs_seamless", "gem": "gem_seamless", "ecmwf": "ecmwf_ifs025"}
MODEL_COLORS = {"hrrr": C_HRRR, "gem": C_GEM, "ecmwf": C_ECMWF}
...
def fetch_metar_history(hours_back=25):
    """METAR temp readings from aviationweather.gov are always Celsius — convert to °F,
    since this market (and this whole dashboard) is Fahrenheit."""
    try:
        r = requests.get("https://aviationweather.gov/api/data/metar",
...
def save_dashboard_snapshot(date, now_str, real_t, models, wind_dir, sky):
    ...
    snap = {
        "city": "New York", "brand": BRAND, "date": date, "updated_at": now_str,
        "updated_at_utc": datetime.now(ZoneInfo("UTC")).isoformat(),
        "unit": "F", "real_temp": real_t, "wind_dir": wind_dir, "sky": sky,
        "models": models,
    }
...
def bucket_probs(center, sigma, pad=15):
...
def fetch_new_york_market_meta(target_date):
    try:
...
def maybe_record_daily_picks(date, probs_by_model, market_meta):
    ...
    msg = [f"📌 NUEVA YORK — PICKS DEL DÍA — {date}"]
    for p in picks:
        msg.append(f"  {p['bucket']}°F @ ${p['entry_price']:.3f}")
    send_telegram("\n".join(msg))
...
def render_dashboard(date, now_str, real_t, finals, peak_hours, wind_dir, sky,
    ...
    axh.text(0, 0.2, f"Nueva York ({STATION}) · {date} · {now_str}", fontsize=13, color=C_TEXT_DIM, transform=axh.transAxes)
    ...
    for i, (label, val, color) in enumerate(cards[:3]):
        ...
        ax.text(0.05, 0.30, f"{val:.0f}°F" if val is not None else "s/d", fontsize=24, color=color,
                fontweight="bold", transform=ax.transAxes)

    ax_4 = fig.add_subplot(gs[2, 0]); _card(ax_4, cards[3][0]); ax_4.axis("off")
    ax_4.text(0.05, 0.30, f"{cards[3][1]:.0f}°F" if cards[3][1] is not None else "s/d", fontsize=21,
    ...
    if avg_diff is not None:
        ...
        wtxt += f"   |   real vs ECMWF: {sign}{avg_diff:.1f}°F ({n_obs} obs)"
    ...
    if chart_rows:
        buckets = [f"{r[0]}°F" for r in chart_rows]
    ...
    path = IMG_DIR / "new_york_dashboard.png"
...
def run_check():
    ...
    print("=" * 60)
    print(f"  NUEVA YORK — HRRR / GEM / ECMWF — {date} ({now_str})")
    print("=" * 60)
    ...
    # No hay tabla de sesgo para Nueva York todavía (se necesita historial acumulado
    ...
    if market_meta:
        ...
        for m in market_meta:
            ...
            print(f"    {bucket}°F  mkt ${price:.3f}  " + " | ".join(f"{n.upper()} {p*100:.0f}% (EV{evs[n]:+.2f})" for n, p in probs_here.items()))
    ...
    real_txt = f"{real_t:.0f}°F" if real_t is not None else "s/d"
    wu_txt = f"  |  WU hoy: {wu_max:.0f}°F" if wu_max is not None else ""
    modelos_txt = " · ".join(
        f"{n.upper()} {finals[n]:.0f}°" if finals.get(n) is not None else f"{n.upper()} s/d"
        for n in MODELS
    )
    lines = [
