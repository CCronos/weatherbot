...
# -*- coding: utf-8 -*-
"""
analyze_buenos_aires_accuracy.py — Desde el inicio del tracking en adelante: pronóstico
ECMWF vs. resultado real en Polymarket para Buenos Aires. Fuente de pronóstico:
buenos_aires_readings.json (mantenido por check_buenos_aires.py).
"""
...
SEARCH_QUERIES = ["buenos aires temperature", "buenos aires", "highest temperature buenos aires", "Buenos Aires°C"]
SMOOTH_N = 3  # must match check_buenos_aires.py's smoothing window

BRAND = "PRECISION REPORT — BUE"
# Mismo tema de Buenos Aires (acento amarillo) que check_buenos_aires.py.
C_BG, C_PANEL, C_ACCENT = "#242a34", "#373d45", "#c98500"
C_WIN, C_LOSS = "#0ca30c", "#d03b3b"
C_TEXT, C_TEXT_DIM, C_GRID = "#ffffff", "#c3c2b7", "#454b52"
...
def fetch_all_buenos_aires_markets():
    found = {}
    seen_market_ids = set()
    total_events = 0
    ...
    for query in SEARCH_QUERIES:
        ...
        for ev in events:
            for m in ev.get("markets", []):
                q = m.get("question", "")
                if "buenos aires" not in q.lower():
                    continue
                mid = m.get("id")
    ...
    print(f"  [DEBUG] {total_events} eventos revisados en {len(SEARCH_QUERIES)} búsquedas | "
          f"{len(seen_market_ids)} mercados únicos de Buenos Aires | fechas encontradas: {sorted(found.keys())}")
...
def get_ecmwf_forecast_for_date(date_str):
    """Prioridad: 1) valor final (suavizado, sin corrección de sesgo — Buenos Aires
    no tiene BIAS_TABLE, a diferencia de Helsinki/Chengdu) 2) fallback: promedio
    suavizado del crudo (días antiguos que no tienen el valor final persistido)."""
    if READINGS_FILE.exists():
        try:
            ...
            if final_key in readings and readings[final_key]:
                return readings[final_key][-1], "check_buenos_aires(final,smoothed)"
            key = f"{date_str}_ecmwf"
            if key in readings and readings[key]:
                recent = readings[key][-SMOOTH_N:]
                return sum(recent) / len(recent), "check_buenos_aires(smoothed)"
        except Exception:
            pass
...
def collect_rows():
    all_markets = fetch_all_buenos_aires_markets()
    rows = []
...
def render_accuracy_dashboard(rows, now_str):
    ...
    axh.text(0, 0.15, f"Buenos Aires · ECMWF vs resultado real · muestra: {len(decided)} · {now_str}",
    ...
    path = IMG_DIR / "buenos_aires_accuracy_dashboard.png"
...
def main():
    now_str = datetime.now(ZoneInfo("America/Argentina/Buenos_Aires")).strftime("%Y-%m-%d %H:%M")
    print("=" * 80)
    print(f"  ANÁLISIS — ECMWF vs RESULTADO REAL (Buenos Aires), desde {START_DATE}")
    print("=" * 80)

    rows = collect_rows()
    if not rows:
        print("\n  ⚠️ No se encontró NINGÚN mercado de Buenos Aires.")
        return
    ...
    send_telegram_photo(dash_path, caption=f"📈 Reporte de precisión ECMWF Buenos Aires (desde {START_DATE}) — {n_hits} aciertos / {n_misses} fallos ({wr:.0f}%)")
