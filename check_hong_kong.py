...
# -*- coding: utf-8 -*-
"""
check_hong_kong.py — Hong Kong: JMA-GSM + ICON-Global + ECMWF (separados, sin promediar)
Este mercado NO resuelve en un aeropuerto — resuelve con la lectura del Hong Kong
Observatory (estación HKO, no VHHH). Por eso la temperatura "real" y el histórico de
24h vienen de la API pública del HKO (data.weather.gov.hk), no de METAR/TAF como en
las demás ciudades — y no hay tarjeta de viento/cielo ni tabla de sesgo (no hay dato
de condición HKO comparable a METAR, ni historial suficiente todavía para calibrarla).
Dashboard visual propio + envío a Telegram | Tracking automático de picks.
"""
...
LAT, LON = 22.3020, 114.1742  # Hong Kong Observatory HQ (King's Park) — the resolution source, not the airport
TZ = "Asia/Hong_Kong"
READINGS_FILE = Path("data/hong_kong_readings.json")
TEMP_HISTORY_FILE = Path("data/hong_kong_temp_history.json")
TRACKING_FILE = Path("data/hong_kong_tracking.json")
STATS_FILE = Path("data/hong_kong_stats.json")
DASH_SNAPSHOT_FILE = Path("data/dashboard_snapshot_hong_kong.json")
...
BRAND = "SIGNAL // HKG"
# Tema único de Hong Kong — acento magenta. Colores de modelo (ECMWF/ICON) fijos en
# todos los dashboards de todas las ciudades, para reconocer el modelo por color sin
# importar la ciudad; JMA-GSM es exclusivo de Asia oriental, no colisiona con nadie más.
C_BG, C_PANEL, C_ACCENT = "#262541", "#39345a", "#d55181"
C_JMA, C_ICON, C_ECMWF = "#c98500", "#9085e9", "#199e70"
C_REAL, C_TEXT, C_TEXT_DIM, C_GRID = "#ffffff", "#ffffff", "#c3c2b7", "#473f66"

MODELS = {"jma_gsm": "jma_gsm", "icon_global": "icon_global", "ecmwf": "ecmwf_ifs025"}
MODEL_COLORS = {"jma_gsm": C_JMA, "icon_global": C_ICON, "ecmwf": C_ECMWF}
...
def fetch_hko_reading():
    """Lectura oficial del Hong Kong Observatory — la fuente de resolución real del
    mercado (NO el aeropuerto VHHH). API pública sin key: data.weather.gov.hk."""
    try:
        r = requests.get("https://data.weather.gov.hk/weatherAPI/opendata/weather.php",
                          params={"dataType": "rhrread", "lang": "en"}, timeout=10)
        data = r.json()
...
def load_temp_history():
    if not TEMP_HISTORY_FILE.exists():
        return []
    try:
        return json.loads(TEMP_HISTORY_FILE.read_text(encoding="utf-8"))
    except Exception as e:
        print(f"  [WARN] {TEMP_HISTORY_FILE.name} corrupto, se ignora: {e}")
        return []


def append_temp_history(value):
    """El HKO no publica una serie horaria pública simple como METAR — se arma un
    historial propio guardando cada lectura de esta corrida, recortado a 25h."""
    hist = load_temp_history()
    now_utc = datetime.now(ZoneInfo("UTC"))
    hist.append([now_utc.isoformat(), value])
    cutoff = now_utc - timedelta(hours=25)
    hist = [[t, v] for t, v in hist if datetime.fromisoformat(t) >= cutoff]
    TEMP_HISTORY_FILE.parent.mkdir(exist_ok=True)
    tmp = TEMP_HISTORY_FILE.with_suffix(TEMP_HISTORY_FILE.suffix + ".tmp")
    tmp.write_text(json.dumps(hist), encoding="utf-8")
    tmp.replace(TEMP_HISTORY_FILE)
    return [(datetime.fromisoformat(t).astimezone(ZoneInfo(TZ)), v) for t, v in hist]
...
def save_dashboard_snapshot(date, now_str, real_t, models):
    ...
    Hong Kong no tiene METAR viento/cielo propio en este script (real_t viene de
    HKO), asi que wind_dir/sky quedan en null a proposito."""
    snap = {
        "city": "Hong Kong", "brand": BRAND, "date": date, "updated_at": now_str,
        "updated_at_utc": datetime.now(ZoneInfo("UTC")).isoformat(),
        "unit": "C", "real_temp": real_t, "wind_dir": None, "sky": None,
        "models": models,
    }
...
def bucket_probs(center, sigma, pad=12):
    ...
    out-of-range forecasts instead of clamping to a fixed window."""
...
def fetch_hong_kong_market_meta(target_date):
    try:
        ...
        for m in r.json():
            ...
            if "hong kong" not in q: continue
...
def maybe_record_daily_picks(date, probs_by_model, market_meta):
    ...
    msg = [f"📌 HONG KONG — PICKS DEL DÍA — {date}"]
    for p in picks:
...
def render_dashboard(date, now_str, real_t, finals, peak_hours,
                      hrs_cycle, cycle_label, fresh, rain_lines,
                      probs_by_model, chart_rows, temp_series):
    ...
    axh.text(0, 0.2, f"Hong Kong (HKO) · {date} · {now_str}", fontsize=13, color=C_TEXT_DIM, transform=axh.transAxes)
    draw_signature(axh, fig)

    # 4 tarjetas (real + 3 modelos) en una grilla de 3 columnas — las primeras 3 en
    # una fila, la 4ta sola en la siguiente (no entran las 4 en una sola fila de 3).
    cards = [("REAL AHORA (HKO)", real_t, C_REAL)] + [
    ...
    for i, (label, val, color) in enumerate(cards[:3]):
        ...
        ax.text(0.05, 0.30, f"{val:.1f}°C" if val is not None else "s/d", fontsize=22, color=color,
                fontweight="bold", transform=ax.transAxes)
    label4, val4, color4 = cards[3]
    ax4 = fig.add_subplot(gs[2, 0]); _card(ax4, label4); ax4.axis("off")
    ax4.text(0.05, 0.30, f"{val4:.1f}°C" if val4 is not None else "s/d", fontsize=22, color=color4,
             fontweight="bold", transform=ax4.transAxes)

    ax_spark = fig.add_subplot(gs[3, :]); _card(ax_spark, "TEMPERATURA REAL — 24H (HKO, muestreo propio)")
    if temp_series and len(temp_series) >= 2:
        ...
        ax_spark.axis("off")
        ax_spark.text(0.03, 0.5, "Datos insuficientes — el historial se arma con cada corrida.",
                       fontsize=12, color=C_TEXT_DIM, transform=ax_spark.transAxes)
    ...
    path = IMG_DIR / "hong_kong_dashboard.png"
...
def run_check():
    ...
    print("=" * 60)
    print(f"  HONG KONG — JMA-GSM / ICON-GLOBAL / ECMWF — {date} ({now_str})")
    print("=" * 60)
    ...
    for name, model_id in MODELS.items():
        ...
        print(f"  {name.upper():11} {peak_t}  a las {peak_h}" if peak_t else f"  {name.upper():11} sin dato")

    # No hay tabla de sesgo para Hong Kong todavía (se necesita historial acumulado
    # para calibrarla, igual que se hizo con Chengdu/Helsinki con el tiempo).
    finals = {}
    ...
    real_t = fetch_hko_reading()
    if real_t is not None:
        print(f"  Temp. real ahora (HKO): {real_t:.1f}°C")
        history = append_temp_history(real_t)
    else:
        history = [(datetime.fromisoformat(t).astimezone(ZoneInfo(TZ)), v)
                    for t, v in load_temp_history()]
    ...
    market_meta = fetch_hong_kong_market_meta(date)
    chart_rows = []
    if market_meta:
        print("\n  MERCADO EN VIVO:")
    ...
    dash_path = render_dashboard(
        date, now_str, real_t, finals, peak_hours,
        hrs_cycle, cycle_label, fresh, rain_lines,
        probs_by_model, chart_rows, temp_series,
    )
    save_dashboard_snapshot(date, now_str, real_t, finals)
...
def main():
    print(f"{BRAND} — iniciado (fuente: Hong Kong Observatory, no aeropuerto)")
